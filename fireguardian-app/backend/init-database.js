/**
 * Script para inicializar la base de datos y poblarla con datos realistas
...
async function seedDatabase() {
  try {
    ...
    for (const tipo of tiposExtintores) {
      await run(
        'INSERT INTO tipos_extintores (id, nombre, descripcion, uso_recomendado, color_hex, icono_path) VALUES (?, ?, ?, ?, ?, ?)',
        [tipo.id, tipo.nombre, tipo.descripcion, tipo.uso_recomendado, tipo.color_hex, tipo.icono_path]
      );
    }
    ...
    for (const ubicacionId of ubicacionesIds) {
      ...
      for (let i = 0; i < numExtintores; i++) {
        ...
        // Guardar extintor
        const result = await run(
          'INSERT INTO extintores (codigo_interno, tipo_id, descripcion, ubicacion_id, responsable_id, fecha_vencimiento, fecha_mantenimiento) VALUES (?, ?, ?, ?, ?, ?, ?)',
